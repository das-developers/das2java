...
public class TableUtil {
     ...
     */
    public static int yTagBinarySearch( TableDataSet ds, int table, Datum datum, int low, int high ) {
        Units units= datum.getUnits();
        double key= datum.doubleValue(units);
        while (low <= high) {
            int mid = (low + high) >> 1;
            double midVal = ds.getYTagDouble(table,mid,units);
            int cmp;
            if (midVal < key) {
	...
	/** Searches the ytags of the specified table of the specified data set for
	 * the specified datum using the binary search algorithm.
	 *
	 * @param ds the data set to search
	 * @param table the table to search
	 * @param datum the key to search for
	 * @return index of the search datum if it exists as an ytag in the
	 *         data set; otherwise the insertion point.  The insertion point is
	 *         is defined as -1.0 if the datum is less that the first ytag
	 *         in the data set, <code>ds.getYLength(table)</code> if the datum
	 *         is larger than the last ytag, or
	 *         <code>(i + (datum - y0) / (y1 - y0) )</code>
	 *         where y0 is the largest ytag smaller than <code>datum</code>,
	 *         y1 is the smallest ytag larger than <code>datum</code>, and
	 *         <code>i</code> is the index of y0.
	 */
   	public static double rowFindex( TableDataSet ds, int table, Datum datum ) {
		int result = yTagBinarySearch( ds, table, datum, 0, ds.getXLength()-1);
		...
		if (result == ds.getYLength(table)) {
			return result;
		}
		else {
			int lowerIndex = result-1;
			double lower = ds.getYTagDouble(table, lowerIndex, u);
			double upper = ds.getYTagDouble(table, result, u);
			double key = datum.doubleValue(u);
			return lowerIndex + (key - lower) / (upper - lower);
		}
	}
