...
public class DataSetUtil {
	...
	/** Searches the xtags of the specified data set for the specified datum
	 * using the binary search algorithm.
	 * 
	 * @param ds the data set to search
	 * @param datum the key to search for
	 * @return index of the search datum if it exists as an xtag in the
	 *         data set; otherwise the insertion point.  The insertion point is
	 *         is defined as -1.0 if the datum is less that the first xtag
	 *         in the data set, <code>ds.getXLength()</code> if the datum is
	 *         larger than the last xtag, or
	 *         <code>(i + (datum - x0) / (x1 - x0) )</code>
	 *         where x0 is the largest xtag smaller than <code>datum</code>,
	 *         x1 is the smallest xtag larger than <code>datum</code>, and
	 *         <code>i</code> is the index of x0.
	 */
	public static double columnFindex( DataSet ds, Datum datum ) {
		int result = xTagBinarySearch( ds, datum, 0, ds.getXLength()-1);
		...
		if (result == ds.getXLength()) {
			return result;
		}
		else {
			Units u = ds.getXUnits();
			int lowerIndex = result-1;
			double lower = ds.getXTagDouble(lowerIndex, u);
			double upper = ds.getXTagDouble(result, u);
