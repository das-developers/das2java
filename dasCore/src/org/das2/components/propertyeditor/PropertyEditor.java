...
public class PropertyEditor extends JComponent {
	...
	public static boolean isBean(Object lbean) {
		try {
			...
			logger.log(Level.FINEST, "isBean true because addPropertyChangeListener exists: {0} {1}",
					new Object[] { lbean, m });
			return true;
	...
	private static void addListenerAndRecurse(Object lbean, PropertyChangeListener thepcl,
			Set<Object> listeningToAlready) {
		...
		for (Method m2 : lbean.getClass().getDeclaredMethods()) {
			if (m2.getName().startsWith("get") && m2.getParameterCount() == 0
					&& (m2.getModifiers() & Modifier.PUBLIC) != 0) {
				try {
					...
					if (child.getClass().isArray()) {
						for (int i = 0; i < Array.getLength(child); i++) {
							...
							if (editableTypes.contains(childi.getClass()))
								continue;
							if (!isBean(childi))
								continue;
							if (listeningToAlready.contains(childi))
								continue;
							addListenerAndRecurse(childi, thepcl, listeningToAlready);
						}
					} else {
						if (editableTypes.contains(child.getClass()))
							continue;
						if (!isBean(child))
							continue;
						if (listeningToAlready.contains(child))
							continue;
	...
	private static void removeListenerAndRecurse(Object lbean, PropertyChangeListener thepcl,
			Set<Object> removedAlready) {
		Method m;
		try {
				...
				try {
					...
					if (child.getClass().isArray()) {
						for (int i = 0; i < Array.getLength(child); i++) {
							...
							if (!isBean(childi))
								continue;
							if (removedAlready.contains(childi))
								continue;
						...
						if (!isBean(child))
							continue;
						if (removedAlready.contains(child))
							continue;
	...
	private PropertyEditor(PropertyTreeNodeInterface root, Object bean) {
		...
		if (table.getColumnCount() > 5) {
			this.setPreferredSize(new Dimension((int) Math.min(Toolkit.getDefaultToolkit().getScreenSize().getWidth(),
					table.getColumnCount() * 100), 500));
		}
	}
	 ...
	 * 
	 * @param leader
	 *            the leader bean, which is used to get settings for all components.
	 * @param peers
	 *            the peers, which must all be instances of the leader's class.
	...
	public static PropertyEditor createPeersEditor(Object leader, Object[] peers) {
		...
		for (Object peer : peers) {
			if (!leaderClass.isInstance(peer)) {
				throw new IllegalArgumentException("child is not instance of leader class: " + peer.getClass().getName()
						+ ", should be " + leaderClass.getName());
			}
		}
	...
	private void addActions(final JTable table) {
		...
		table.getInputMap().put(KeyStroke.getKeyStroke(KeyEvent.VK_ENTER, 0, false), "MY_EDIT");
	}
				...
				if (selected.length == 0) {
					...
					if (node instanceof IndexedPropertyTreeNode) {
						...
						for (int i = 0; i < kids.size(); i++) {
							...
							if (leader.getClass().isInstance(peer))
								peers.add(peer);
						}
					...
					if (peers.size() == 1 && peers.get(0) == leader) { // bug where hidden selected parent is leader
						...
						if (newLeader == leader)
							newLeader = ((PropertyTreeNode) model.getNodeForRow(selected[1])).getValue();
						leader = newLeader;
						for (int i = 0; i < selected.length; i++) {
							Object peer = ((PropertyTreeNode) model.getNodeForRow(selected[i])).getValue();
							if (leader.getClass().isInstance(peer))
	...
	private void dismissDialog(boolean checkDirty) {
		...
		if (checkDirty && root.isDirty()) {
			...
			int result = JOptionPane.showConfirmDialog(this, message, "", JOptionPane.YES_NO_CANCEL_OPTION,
	...
	public JDialog getDialog(Component c) {
		if (dialog == null) {
			...
			if (c != null) {
				...
				if (top == null && c instanceof JFrame)
					top = (JFrame) c;
			}
			if (top instanceof JFrame) {
				dialog = new JDialog((JFrame) top);
	 ...
	 *            the dialog title.
	 * @param icon
	 *            the icon branding the application.
	 */
